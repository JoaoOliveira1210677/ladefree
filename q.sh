...
print_header() {
    clear
    echo -e "${GREEN}======================================================${NC}"
    echo -e "${GREEN}          Python Xray Argo 一键部署脚本 V2.0          ${NC}"
    echo -e "${GREEN}======================================================${NC}"
    echo
    echo -e "${MAGENTA}[*] 基于项目: ${CYAN}https://github.com/eooce/python-xray-argo${NC}"
    echo -e "${MAGENTA}[*] 脚本仓库: ${CYAN}https://github.com/byJoey/free-vps-py${NC}"
    echo -e "${MAGENTA}[*] TG交流群:  ${CYAN}https://t.me/+ft-zI76oovgwNmRh${NC}"
    echo -e "${RED}[♥] 作者YouTube: ${CYAN}https://www.youtube.com/@joeyblog${NC}"
    echo
    echo -e "本脚本提供极速与完整两种模式，支持HF API保活与分流。"
    echo
}
...
if [ "$1" = "-v" ]; then
    if [ -f "$NODE_INFO_FILE" ]; then
        ...
        echo -e "${RED}[X] 未找到节点信息文件: $NODE_INFO_FILE${NC}"
        echo -e "${CYAN}[!] 请先运行部署脚本生成节点信息。${NC}"
    fi
    exit 0
fi
...
echo -e "  ${MAGENTA}1)${NC} 极速模式 - 仅配置核心选项，快速启动"
echo -e "  ${MAGENTA}2)${NC} 完整模式 - 可配置所有高级功能"
echo -e "  ${MAGENTA}3)${NC} 查看节点 - 显示已保存的节点部署信息"
echo -e "  ${MAGENTA}4)${NC} 查看保活 - 检查Hugging Face API保活状态"
echo
...
if [ "$MODE_CHOICE" = "3" ]; then
    if [ -f "$NODE_INFO_FILE" ]; then
        ...
        echo
        echo -e "${GREEN}==================================================${NC}"
        echo -e "${CYAN}[!] 如需重新部署，请再次运行脚本并选择模式1或2。${NC}"
        exit 0
    else
        echo
        echo -e "${RED}[X] 未找到节点信息文件。${NC}"
        echo -e "${CYAN}[!] 请先运行部署脚本以生成节点信息。${NC}"
        echo
        read -p "是否现在开始部署? (y/n): " START_DEPLOY
        if [[ "$START_DEPLOY" != "y" && "$START_DEPLOY" != "Y" ]]; then
            echo -e "${GREEN}[+] 操作取消，退出脚本。${NC}"
            exit 0
        fi
        print_header
        echo -e "${CYAN}请选择部署模式:${NC}"
        echo -e "  ${MAGENTA}1)${NC} 极速模式"
        echo -e "  ${MAGENTA}2)${NC} 完整模式"
...
if [ "$MODE_CHOICE" = "4" ]; then
    ...
    if [ -n "$KEEPALIVE_PID" ]; then
        ...
        echo -e "  - 进程PID: ${CYAN}$KEEPALIVE_PID${NC}"
        if [ -f "keep_alive_task.sh" ]; then
            REPO_ID=$(grep 'huggingface.co/api/' keep_alive_task.sh | head -1 | sed -n 's|.*api/.*/\([^"]*\).*|\1|p')
            REPO_TYPE=$(grep 'huggingface.co/api/' keep_alive_task.sh | head -1 | sed -n 's|.*api/\([^/]*\).*|\1|p')
            echo -e "  - 目标仓库: ${CYAN}$REPO_ID${NC} (类型: $REPO_TYPE)"
        fi
        echo
        echo -e "${MAGENTA}[*] 最近一次保活状态:${NC}"
        echo "--------------------------------------------------"
        if [ -f "keep_alive_status.log" ]; then
           cat keep_alive_status.log
        else
           echo -e "${CYAN}[!] 尚未生成状态日志，请稍等片刻(最多2分钟)后重试...${NC}"
        fi
        echo "--------------------------------------------------"
    else
        echo -e "${RED}[X] 服务状态: 未运行${NC}"
        echo -e "${CYAN}[!] 提示: 您可能尚未部署服务或未在部署时设置Hugging Face保活。${NC}"
    fi
    echo
    exit 0
fi
...
echo -e "${MAGENTA}[*] 检查并安装依赖...${NC}"
if ! command -v python3 &> /dev/null; then
    echo -e "${CYAN}  - 正在安装 Python3...${NC}"
    sudo apt-get update && sudo apt-get install -y python3 python3-pip > /dev/null 2>&1
fi
if ! python3 -c "import requests" &> /dev/null; then
    echo -e "${CYAN}  - 正在安装 Python 依赖: requests...${NC}"
    pip3 install requests > /dev/null 2>&1
fi

if [ ! -d "$PROJECT_DIR_NAME" ]; then
    echo -e "${MAGENTA}[*] 下载项目仓库...${NC}"
    if command -v git &> /dev/null; then
        git clone https://github.com/eooce/python-xray-argo.git "$PROJECT_DIR_NAME" > /dev/null 2>&1
    else
        echo -e "${CYAN}  - Git未安装，使用wget下载...${NC}"
        wget -q https://github.com/eooce/python-xray-argo/archive/refs/heads/main.zip -O temp.zip
        unzip -q temp.zip && mv python-xray-argo-main "$PROJECT_DIR_NAME" && rm temp.zip
    fi
    if [ $? -ne 0 ]; then echo -e "${RED}[X] 下载失败!${NC}"; exit 1; fi
fi

cd "$PROJECT_DIR_NAME"
echo -e "${GREEN}[+] 依赖与项目文件准备就绪!${NC}"
echo

if [ ! -f "app.py" ]; then
    echo -e "${RED}[X] 未找到核心文件 app.py!${NC}"; exit 1;
fi

cp app.py app.py.backup
echo -e "${CYAN}[!] 已备份原始文件为 app.py.backup${NC}"
...
configure_hf_keep_alive() {
    echo
    read -p "是否设置 Hugging Face API 自动保活? (y/n): " SETUP_KEEP_ALIVE
    if [[ "$SETUP_KEEP_ALIVE" = "y" || "$SETUP_KEEP_ALIVE" = "Y" ]]; then
        echo -e "${CYAN}请输入您的 Hugging Face 访问令牌 (Token):${NC}"
        echo -e "（提示: 令牌输入时不可见，可从 https://huggingface.co/settings/tokens 获取 'read' 权限令牌）"
        read -sp "Token: " HF_TOKEN_INPUT
        echo
        if [ -z "$HF_TOKEN_INPUT" ]; then echo -e "${RED}[X] Token 不能为空，已取消保活。${NC}"; return; fi
        read -p "请输入要访问的 HF 仓库ID (例如: joeyhuangt/aaaa): " HF_REPO_ID_INPUT
        if [ -z "$HF_REPO_ID_INPUT" ]; then echo -e "${RED}[X] 仓库ID 不能为空，已取消保活。${NC}"; return; fi
        HF_TOKEN="$HF_TOKEN_INPUT"
        HF_REPO_ID="$HF_REPO_ID_INPUT"
        KEEP_ALIVE_HF="true"
        echo -e "${GREEN}[+] Hugging Face API 保活已设置! 目标仓库: $HF_REPO_ID${NC}"
    fi
}

if [ "$MODE_CHOICE" = "1" ]; then
    print_header
    echo -e "${GREEN}============= 极速模式配置 =============${NC}"
    echo
    read -p "请输入新的 UUID (留空自动生成): " UUID_INPUT
    if [ -z "$UUID_INPUT" ]; then
        UUID_INPUT=$(generate_uuid)
        echo -e "${CYAN}[!] 自动生成UUID: $UUID_INPUT${NC}"
    fi
    ...
    print_header
    echo -e "${GREEN}============= 完整模式配置 =============${NC}"
    echo
    read -p "请输入新的 UUID (留空自动生成): " UUID_INPUT
    if [ -z "$UUID_INPUT" ]; then UUID_INPUT=$(generate_uuid); echo -e "${CYAN}[!] 自动生成UUID: $UUID_INPUT${NC}"; fi
    sed -i "s/UUID = os.environ.get('UUID', '[^']*')/UUID = os.environ.get('UUID', '$UUID_INPUT')/" app.py
    
    read -p "请输入节点名称 (留空默认): " NAME_INPUT
    if [ -n "$NAME_INPUT" ]; then sed -i "s/NAME = os.environ.get('NAME', '[^']*')/NAME = os.environ.get('NAME', '$NAME_INPUT')/" app.py; fi
...
APP_PID=$!
sleep 2
if ! ps -p $APP_PID > /dev/null; then APP_PID=$(pgrep -f "python3 app.py" | head -1); fi

if [ -z "$APP_PID" ]; then
    echo -e "${RED}[X] 主服务启动失败! 请检查日志: tail -f $(pwd)/app.log${NC}"
    exit 1
fi
echo -e "${GREEN}[+] 主服务已在后台启动 (PID: $APP_PID)。${NC}"

KEEPALIVE_PID=""
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    echo -e "${MAGENTA}[*] 正在创建并启动 Hugging Face API 保活任务...${NC}"
    ...
    KEEPALIVE_PID=$!
    echo -e "${GREEN}[+] API 保活任务已启动 (PID: $KEEPALIVE_PID)。${NC}"
fi

echo -ne "${CYAN}[*] 正在等待Argo隧道建立与节点生成, 请耐心等待... ${NC}"
MAX_WAIT=600; WAIT_COUNT=0; NODE_INFO=""
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    if [ -f ".cache/sub.txt" ] && [ -s ".cache/sub.txt" ]; then NODE_INFO=$(cat .cache/sub.txt); fi
    if [ -f "sub.txt" ] && [ -s "sub.txt" ]; then NODE_INFO=$(cat sub.txt); fi
    if [ -n "$NODE_INFO" ]; then echo -e "${GREEN}完成!${NC}"; break; fi
    echo -n "."
    sleep 5
...
if [ -z "$NODE_INFO" ]; then
    echo -e "${RED}失败!${NC}"
    echo -e "${RED}[X] 等待超时！节点信息未能在10分钟内生成。${NC}"
    echo -e "${CYAN}[!] 请检查日志: tail -f $(pwd)/app.log${NC}"
    exit 1
fi
...
echo
echo -e "${CYAN}核心订阅地址:${NC}"
echo "+-------------------------------------------------+"
echo -e "| ${GREEN}${PUBLIC_IP}:3000/${SUB_PATH_VALUE}${NC}"
echo "+-------------------------------------------------+"
echo
echo -e "${MAGENTA}[*] 服务详情:${NC}"
echo -e "  - 主服务 PID: ${CYAN}$APP_PID${NC}"
if [ -n "$KEEPALIVE_PID" ]; then echo -e "  - 保活服务 PID: ${CYAN}$KEEPALIVE_PID${NC}"; fi
echo -e "  - UUID: ${CYAN}$CURRENT_UUID${NC}"
echo
echo -e "${MAGENTA}[*] 节点订阅内容 (Base64):${NC}"
echo "$NODE_INFO"
echo
...
[*] 服务详情:
  - 主服务 PID: $APP_PID
"
if [ -n "$KEEPALIVE_PID" ]; then SAVE_INFO="${SAVE_INFO}  - 保活服务 PID: $KEEPALIVE_PID
"; fi
SAVE_INFO="${SAVE_INFO}  - UUID: $CURRENT_UUID
  - 订阅路径: /$SUB_PATH_VALUE
--------------------------------------------------
[*] 订阅地址:
  - 公网: http://${PUBLIC_IP}:3000/${SUB_PATH_VALUE}
  - 本地: http://localhost:3000/${SUB_PATH_VALUE}
--------------------------------------------------
[*] 管理命令:
...
"
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    SAVE_INFO="${SAVE_INFO}  - 停止保活服务: pkill -f keep_alive_task.sh && rm keep_alive_task.sh keep_alive_status.log
"
fi
SAVE_INFO="${SAVE_INFO}==================================================
"
...
echo -e "${GREEN}[+] 节点信息已保存至 ${CYAN}$NODE_INFO_FILE${NC}"
echo -e "${CYAN}[!] 您可以使用带 '-v' 参数的命令随时查看。${NC}"
echo
echo -e "${GREEN}部署完成！感谢使用！${NC}"

exit 0if [ "$MODE_CHOICE" = "3" ]; then
    if [ -f "$NODE_INFO_FILE" ]; then
        echo
        echo -e "${GREEN}========================================${NC}"
