...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    ENABLE_YOUTUBE_DIVERSION=true
    
else
    ENABLE_YOUTUBE_DIVERSION=false
    echo
...
if [ "$ENABLE_YOUTUBE_DIVERSION" = "true" ]; then
...
if [ -n "$NODE_INFO" ]; then
    ...
    YOUTUBE_NODE="vmess:"
    echo -e "${GREEN}YouTube分流节点:${NC}"
    echo "$YOUTUBE_NODE"
    echo
    ...
    echo -e "${GREEN}完整订阅链接 (包含YouTube分流，Base64编码):${NC}"
    echo "$COMBINED_BASE64"
    echo
    
    echo -e "${GREEN}原始订阅链接 (Base64编码):${NC}"
    echo "$NODE_INFO"
    echo
else
    echo -e "${YELLOW}=== 节点信息 ===${NC}"
...
echo -e "${GREEN}部署完成！感谢使用！${NC}"echo
read -p "请输入选择 (1/2): " MODE_CHOICE
