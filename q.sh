            ...
            if [ "$AUTO_ACCESS" = "true" ]; then
                ...
                if [ "$TEST_KEEPALIVE" = "y" ] || [ "$TEST_KEEPALIVE" = "Y" ]; then
                    ...
                    if [ -f ".cache/list.txt" ]; then
                        TUNNEL_DOMAIN=$(grep -o "https://[^/]*\.trycloudflare\.com" .cache/list.txt | head -1 | sed 's|https://||')
                    elif [ -f "list.txt" ]; then
                        TUNNEL_DOMAIN=$(grep -o "https://[^/]*\.trycloudflare\.com" list.txt | head -1 | sed 's|https://||')
                    fi
                    
                    if [ -n "$TUNNEL_DOMAIN" ]; then
                        echo -e "${BLUE}隧道域名: ${YELLOW}$TUNNEL_DOMAIN${NC}"
                        echo -e "${BLUE}发送保活请求...${NC}"
                        
                        RESPONSE=$(curl -s -w "\n状态码: %{http_code}\n响应时间: %{time_total}s\n" "https://$TUNNEL_DOMAIN" --max-time 10 2>&1)
                        echo -e "${CYAN}保活响应:${NC}"
                        echo "$RESPONSE"
                        
                        HTTP_CODE=$(echo "$RESPONSE" | grep "状态码:" | cut -d" " -f2)
...
if 'requests.get(project_url' in content:
    ...
    new_keepalive = '''log_keepalive(f"保活请求: {project_url}")
                    ...
                    if response.status_code == 200:
                        ...
                        log_keepalive(f"保活失败: 状态码 {response.status_code}", "ERROR")'''
...
print("YouTube分流配置、80端口节点和保活日志功能已成功添加")
EOF
