...
echo
echo -e "${GREEN}本脚本基于 eooce 大佬的项目进行修改和增强${NC}"
echo -e "${GREEN}提供极速和完整两种配置模式，简化部署流程${NC}"
echo -e "${GREEN}核心分流规则已升级为 Geosite，覆盖更全面${NC}"
...
echo -e "${BLUE}正在应用Geosite分流规则和80端口节点...${NC}"
...
echo -e "${GREEN}Geosite分流规则和80端口节点已集成${NC}"
...
=== 分流说明 ===
- 已集成Geosite分流规则，优化常见服务访问
- Google,YouTube,Telegram,Instagram等将走专用线路
- 无需额外配置，透明分流"
...
echo -e "${GREEN}Geosite分流已集成到xray配置，无需额外设置${NC}"
...
GLOBAL_PROXY="false"
echo
echo -e "${YELLOW}是否启用全局代理模式? (y/n) [默认: n]${NC}"
echo -e "${BLUE}(选择 'y' 将所有流量通过节点代理，可解决部分网站无法访问的问题)${NC}"
echo -e "${BLUE}(选择 'n' 将仅对YouTube等视频流量分流，其他流量直连，适合VPS网络质量好的情况)${NC}"
read -p "> " ENABLE_GLOBAL_PROXY
if [ "$ENABLE_GLOBAL_PROXY" = "y" ] || [ "$ENABLE_GLOBAL_PROXY" = "Y" ]; then
    GLOBAL_PROXY="true"
    echo -e "${GREEN}全局代理模式已启用。${NC}"
else
    GLOBAL_PROXY="false"
    echo -e "${GREEN}YouTube分流模式已启用 (默认)。${NC}"
fi
echo
...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    if [ "$GLOBAL_PROXY" = "true" ]; then
        echo -e "${GREEN}全局代理模式已配置${NC}"
    else
...
echo -e "订阅路径: $(grep "SUB_PATH = " app.py | cut -d"'" -f4)"
if [ "$GLOBAL_PROXY" = "true" ]; then
    echo -e "代理模式: ${GREEN}全局代理${NC}"
else
    echo -e "代理模式: ${GREEN}YouTube分流${NC}"
fi
if [ "$KEEP_ALIVE_HF" = "true" ]; then
    echo -e "保活仓库: $HF_REPO_ID"
fi
...
echo -e "${BLUE}正在根据您的选择配置代理模式和80端口节点...${NC}"
export GLOBAL_PROXY_ENABLED="$GLOBAL_PROXY"
cat > youtube_patch.py << 'EOF'
# coding: utf-8
import os, base64, json, subprocess, time
...
# 从环境变量判断是否启用全局代理
global_proxy_enabled = os.environ.get('GLOBAL_PROXY_ENABLED', 'false') == 'true'
...
print("代理配置和80端口节点已成功添加")
EOF

python3 youtube_patch.py
unset GLOBAL_PROXY_ENABLED
rm youtube_patch.py

echo -e "${GREEN}代理模式和80端口节点已集成${NC}"
...
echo -e "${YELLOW}=== 重要提示 ===${NC}"
if [ "$GLOBAL_PROXY" = "true" ]; then
    echo -e "${GREEN}全局代理模式已启用，所有流量将通过节点${NC}"
else
    echo -e "${GREEN}YouTube分流模式已启用，优化视频体验${NC}"
fi
echo -e "${GREEN}部署已完成，节点信息已成功生成${NC}"
echo -e "${GREEN}可以立即使用订阅地址添加到客户端${NC}"
echo -e "${GREEN}服务将持续在后台运行${NC}"
