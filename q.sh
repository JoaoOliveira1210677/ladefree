...
if [ "${KEEPALIVE_MODE:-auto}" = "disabled" ]; then
    echo -e "${YELLOW}⚠️  保活功能已禁用，跳过保活服务启动${NC}"
else
    # 创建保活脚本
    if [ "${KEEPALIVE_MODE:-auto}" = "manual" ] && [ -n "${KEEPALIVE_URL:-}" ]; then
        create_keepalive_script "manual" "$KEEPALIVE_URL"
    else
        create_keepalive_script "auto"
    fi
    ...
    if [ -n "$KEEPALIVE_PID" ] && ps -p "$KEEPALIVE_PID" > /dev/null 2>&1; then
        ...
        if [ "${KEEPALIVE_MODE:-auto}" = "manual" ]; then
            echo -e "${BLUE}保活模式: 手动配置URL - ${YELLOW}${KEEPALIVE_URL}${NC}"
        else
            echo -e "${BLUE}保活模式: 自动从节点信息提取host${NC}"
        fi
        echo -e "${BLUE}保活间隔: 每2分钟执行一次${NC}"
    else
        echo -e "${YELLOW}⚠️  保活服务可能未正常启动，请稍后检查${NC}"
    fi
fi
...
echo -e "${BLUE}  bash $0 -v     # 查看节点信息${NC}"
echo -e "${BLUE}  bash $0 -h     # 显示帮助信息${NC}"
echo -e "${BLUE}  重新运行脚本选择选项4查看保活状态${NC}"
echo -e "${BLUE}  重新运行脚本选择选项5查看实时日志${NC}"

# 退出脚本
exit 0 -e "${GREEN}订阅路径已设置为: $SUB_PATH_INPUT${NC}"
    fi
...
show_keepalive_status() {
    ...
    if [ "$TEST_KEEPALIVE" = "y" ] || [ "$TEST_KEEPALIVE" = "Y" ]; then
        ...
        if [ -f "$NODE_INFO_FILE" ]; then
            # 从节点信息中提取host参数
            TUNNEL_DOMAIN=$(grep -o "host=[^&]*" "$NODE_INFO_FILE" | head -1 | cut -d"=" -f2)
        ...
        if [ -n "$TUNNEL_DOMAIN" ]; then
            ...
            if curl -s --connect-timeout 10 --max-time 15 "http://$TUNNEL_DOMAIN" > /dev/null 2>&1; then
                echo -e "${GREEN}✓ HTTP请求成功${NC}"
                HTTP_SUCCESS=true
            else
                echo -e "${RED}✗ HTTP请求失败${NC}"
            fi
            
            if curl -s --connect-timeout 10 --max-time 15 "https://$TUNNEL_DOMAIN" > /dev/null 2>&1; then
                echo -e "${GREEN}✓ HTTPS请求成功${NC}"
                HTTPS_SUCCESS=true
            else
                echo -e "${RED}✗ HTTPS请求失败${NC}"
            fi
            
            if [ "$HTTP_SUCCESS" = true ] || [ "$HTTPS_SUCCESS" = true ]; then
                echo -e "${GREEN}✓ 保活测试成功${NC}"
                # 记录到日志
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] 手动测试: 保活请求 $TUNNEL_DOMAIN" >> "$KEEPALIVE_LOG_FILE"
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] 手动测试: 保活成功" >> "$KEEPALIVE_LOG_FILE"
            else
                echo -e "${RED}✗ 保活测试失败${NC}"
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] 手动测试: 保活失败" >> "$KEEPALIVE_LOG_FILE"
            fi
        else
            echo -e "${RED}✗ 无法获取隧道域名${NC}"
        fi
    fi
    ...
    case $LOG_CHOICE in
        1)
            ...
            if [ -n "$WORK_DIR" ] && [ -f "$WORK_DIR/app.log" ]; then
                ...
                echo -e "${YELLOW}日志文件: $WORK_DIR/app.log${NC}"
                echo
        ...
        2)
            if [ -f "$KEEPALIVE_LOG_FILE" ]; then
                ...
                echo -e "${YELLOW}日志文件: $KEEPALIVE_LOG_FILE${NC}"
                echo
...
create_keepalive_script() {
    local keepalive_mode="${1:-auto}"
    local keepalive_url="${2:-}"
...
keepalive() {
    ...
    log_message "保活请求: \$target_url"
    
    # 尝试请求
    if command -v curl &> /dev/null; then
        HTTP_CODE=\$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 10 --max-time 15 "\$target_url" 2>/dev/null)
        
        if [ "\$HTTP_CODE" = "200" ] || [ "\$HTTP_CODE" = "404" ] || [ "\$HTTP_CODE" = "400" ]; then
            log_message "保活成功: \$target_url (状态码: \$HTTP_CODE)"
            return 0
        elif [ -n "\$HTTP_CODE" ] && [ "\$HTTP_CODE" != "000" ]; then
            log_message "保活响应: \$target_url (状态码: \$HTTP_CODE)"
            return 0
        else
            # 如果HTTPS失败，尝试HTTP（仅适用于自动模式）
            if [[ "\$target_url" == https://* ]] && [ "\$KEEPALIVE_MODE" = "auto" ]; then
    ...
    log_message "保活失败: \$target_url"
    return 1
}

# 主循环 - 每2分钟执行一次
if [ "\$KEEPALIVE_MODE" = "manual" ]; then
    log_message "保活服务启动 - 手动模式，目标URL: \$MANUAL_URL"
else
    log_message "保活服务启动 - 自动模式，每2分钟执行一次curl请求"
fi

log_message "正在获取目标URL..."

while true; do
    TARGET_URL=\$(get_node_host)
    
    if [ -n "\$TARGET_URL" ]; then
        keepalive "\$TARGET_URL"
    else
        if [ "\$KEEPALIVE_MODE" = "manual" ]; then
            log_message "手动模式: 配置的URL为空"
        else
            log_message "自动模式: 未找到节点host，等待下次检测..."
        fi
    fi
...
EOF
    ...
    if [ "$keepalive_mode" = "manual" ] && [ -n "$keepalive_url" ]; then
        echo -e "${BLUE}保活模式: 手动配置${NC}"
        echo -e "${BLUE}保活URL: ${YELLOW}$keepalive_url${NC}"
    elif [ "$keepalive_mode" = "auto" ]; then
        echo -e "${BLUE}保活模式: 自动从节点信息提取host${NC}"
    fi
} "$HOME/.xray_nodes_info" ]; then
        # 方法1: 从节点配置中提取host参数
        NODE_HOST=$(grep -o 'host=[^&]*' "$HOME/.xray_nodes_info" | head -1 | cut -d'=' -f2)
        if [ -n "$NODE_HOST" ]; then
            echo "$NODE_HOST"
...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    case "$KEEPALIVE_CHOICE" in
        "1")
            echo -e "${GREEN}✓ 将使用自动保活 - 从节点信息提取host${NC}"
            KEEPALIVE_MODE="auto"
            ;;
        "2")
            echo -e "${YELLOW}手动配置保活URL:${NC}"
            read -p "请输入保活URL (如: https://example.com): " MANUAL_KEEPALIVE_URL
            if [ -n "$MANUAL_KEEPALIVE_URL" ]; then
                echo -e "${GREEN}✓ 手动保活URL已设置为: $MANUAL_KEEPALIVE_URL${NC}"
                KEEPALIVE_MODE="manual"
                KEEPALIVE_URL="$MANUAL_KEEPALIVE_URL"
            else
                echo -e "${YELLOW}未输入URL，将使用自动保活${NC}"
                KEEPALIVE_MODE="auto"
            fi
            ;;
        "3")
    ...
    case "$KEEPALIVE_CHOICE" in
        ...
        "2")
            echo -e "${YELLOW}手动配置保活URL:${NC}"
            echo -e "${BLUE}示例格式:${NC}"
            echo -e "  https://your-tunnel-domain.trycloudflare.com"
            echo -e "  http://example.com"
            echo -e "  https://api.example.com/health"
            echo
            read -p "请输入保活URL: " MANUAL_KEEPALIVE_URL
            if [ -n "$MANUAL_KEEPALIVE_URL" ]; then
                echo -e "${GREEN}✓ 手动保活URL已设置为: $MANUAL_KEEPALIVE_URL${NC}"
                KEEPALIVE_MODE="manual"
                KEEPALIVE_URL="$MANUAL_KEEPALIVE_URL"
                ...
                # 验证URL格式
                if [[ "$MANUAL_KEEPALIVE_URL" =~ ^https?:// ]]; then
                    echo -e "${GREEN}✓ URL格式验证通过${NC}"
                else
                    echo -e "${YELLOW}⚠ URL格式可能不正确，建议使用 http:// 或 https:// 开头${NC}"
                fi
            else
                echo -e "${YELLOW}未输入URL，将使用自动保活${NC}"
                KEEPALIVE_MODE="auto"
    ...
    if [ -n "$SUB_PATH_INPUT" ]; then
        sed -i "s/SUB_PATH = os.environ.get('SUB_PATH', '[^']*')/SUB_PATH = os.environ.get('SUB_PATH', '$SU    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}           保活状态监控               ${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo
